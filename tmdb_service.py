...
def get_tmdb_movie_by_imdb(imdb_id: str) -> Optional[dict]:
    ...
    logger.info(f"[DEBUG] Looking up IMDB ID: {imdb_id}")
    results = find_api.find(imdb_id, external_source='imdb_id')
    logger.info(f"[DEBUG] Find results: {results}")
    if results and 'movie_results' in results and results['movie_results']:
        tmdb_id = results['movie_results'][0]['id']
        logger.info(f"[DEBUG] Found TMDB ID: {tmdb_id}")
        details = movie_api.details(tmdb_id)
        logger.info(f"[DEBUG] Raw TMDB details type: {type(details)}")
        details_dict = asdict_safe(details)
        logger.info(f"[DEBUG] Converted details keys: {list(details_dict.keys()) if details_dict else 'None'}")
        logger.info(f"[DEBUG] Poster path in details: {details_dict.get('poster_path') if details_dict else 'N/A'}")
        return details_dict
    logger.info(f"[DEBUG] No movie results found for IMDB ID: {imdb_id}")
    return None
...
def get_collection_movies_by_tmdb_id(collection_id: int) -> List[dict]:
    ...
    print(f"[DEBUG] Fetching TMDB collection with ID: {collection_id}")
    collection = collection_api.details(collection_id)
    print(f"[DEBUG] Raw TMDB collection response (type: {type(collection)}): {repr(collection)}")
    parts = getattr(collection, 'parts', None)
    if parts is not None:
        print(f"[DEBUG] Collection parts: {parts}")
...
def get_collection_details_by_tmdb_id(collection_id: int) -> Optional[dict]:
    ...
    try:
        print(f"[DEBUG] Fetching TMDB collection details for ID: {collection_id}")
        collection = collection_api.details(collection_id)
        collection_dict = asdict_safe(collection)
        print(f"[DEBUG] Collection details: {collection_dict}")
        return collection_dict
    except Exception as e:
        print(f"[DEBUG] Error fetching collection details: {e}")
        return None
...
def get_collection_movies_by_imdb(imdb_id: str) -> List[dict]:
    ...
    print("[DEBUG] TMDB movie for IMDB ID", imdb_id, ":", tmdb_movie)
    if not tmdb_movie:
        print("[DEBUG] No TMDB movie found for IMDB ID", imdb_id)
        return []
    collection = tmdb_movie.get('belongs_to_collection')
    print("[DEBUG] Collection for movie:", collection)
    if collection and 'id' in collection:
        movies = get_collection_movies_by_tmdb_id(collection['id'])
        print("[DEBUG] Movies in collection:", movies)
        return movies
    print("[DEBUG] No collection found, returning just the movie itself.")
...
async def get_all_episodes_by_imdb_async(imdb_id: str) -> List[dict]:
    ...
    for season in details_dict.get('seasons', []):
        ...
        if season_number == 0:
            print(f"[DEBUG] Skipping season 0 (specials/webisodes) for TMDB series {tmdb_id}")
            continue
    ...
    async def fetch_season(session: aiohttp.ClientSession, season_number: int) -> List[dict]:
        ...
        try:
            async with session.get(url) as resp:
                if resp.status != 200:
                    print(f"[DEBUG] Failed to fetch season {season_number} for TMDB series {tmdb_id}: {resp.status}")
                    return []
                ...
                for ep in season_data.get('episodes', []):
                    ...
                    if season_number is None or episode_val is None:
                        print(f"[DEBUG] Skipping episode with missing season/episode number: {ep}")
                        continue
            ...
            print(f"[DEBUG] Error fetching season {season_number}: {e}")
            return []
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for result in results:
            if isinstance(result, list):
                ...
                print(f"[DEBUG] Error in concurrent fetch: {result}")
...
def get_all_episodes_by_imdb(imdb_id: str) -> List[dict]:
    ...
    try:
        ...
        print(f"[DEBUG] Error in async episode fetch, falling back to sync: {e}")
...
def get_all_episodes_by_imdb_sync(imdb_id: str) -> List[dict]:
    ...
    for season in details_dict.get('seasons', []):
        ...
        if resp.status_code != 200:
            print(f"[DEBUG] Failed to fetch season {season_number} for TMDB series {tmdb_id}: {resp.status_code}")
            continue
