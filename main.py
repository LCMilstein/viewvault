...
from models import Series, Episode, Movie, MovieCreate, SeriesCreate, EpisodeCreate, User, UserCreate, UserLogin, Token, List, ListItem, ListPermission, ListCreate, ListUpdate, ListItemAdd, ListItemUpdate
...
app = FastAPI(lifespan=lifespan, title="CineSync", version="1.0.0")
...
def api_root():
    ...
    return {"message": "CineSync API", "version": "1.0.0", "test": "UPDATED_CODE_RUNNING"}
...
def import_movie(imdb_id: str, current_user: User = Depends(get_current_user)):
    """Import a movie from IMDB by ID"""
    ...
    with Session(engine) as session:
        ...
        if existing:
            raise HTTPException(status_code=400, detail="Movie already exists in your watchlist")
...
def import_movie_with_sequels(imdb_id: str, current_user: User = Depends(get_current_user)):
    """Import a movie and all its sequels from TMDB (franchise/collection)."""
...
def import_series(imdb_id: str, current_user: User = Depends(get_current_user)):
    """Import a TV series from IMDB by ID with all episodes"""
    ...
    with Session(engine) as session:
        ...
        if existing_series:
            raise HTTPException(status_code=400, detail="Series already exists in watchlist")
...
def import_full_series(imdb_id: str, current_user: User = Depends(get_current_user)):
    """Import a TV series with ALL seasons and episodes from IMDB"""
    # Handle both IMDB IDs and TMDB IDs with tmdb_ prefix
    if imdb_id.startswith('tmdb_'):
        # Extract TMDB ID from tmdb_ prefix
        tmdb_id = imdb_id.replace('tmdb_', '')
